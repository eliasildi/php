...
$fp = fopen($nombre, "r") or die ("No existe el archivo \"$nombre\"<br />"); //si fopen es TRUE no mira el OR.
...
$nombrefichero = "archivo01.txt";
$fp = fopen($nombrefichero, "r") or die ("No existe el archivo \"$nombrefichero\"<br />");
while (! feof($fp) ) { // feof, detecta cuando se ha acabado el fichero y devuelve TRUE
	$leido = fgets($fp, 20); //es lo mismo poner 20 de 1024, porque el buffer de almacenamiento es 1024, 2048......
	echo $leido;
}
echo "<br /><br />Fin de la ejecución<br />";
...
echo "<br />Uso de la función fread()<br /><br />"; // se come los intros y sigue dando 4046 hasta que se acaba el fichero
$nombrefichero = "archivo01.txt";
$fp = fopen($nombrefichero, "r") or die ("No existe el archivo \"$nombrefichero\"<br />");
while (! feof($fp) ) {
	$leido = fread($fp, 4096);
...
while (! feof($fp1) ) {
	$leido = fgets($fp1, 4096); //le 4096 caracteres
	fwrite($fp2, $leido); //y lo copia en otro fichero que crea ( copia.txt)
}
fclose($fp1);
fclose($fp2); //hacemos esto para liberar el espacio de memoria y no consuma recursos del sistema.
